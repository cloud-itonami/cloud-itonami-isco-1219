(ns business-admin.actor
  "AdminManagementActor — the ISCO-08 1219 independent business-
  administration-management actor as a `langgraph.graph/state-graph`
  (per ADR-2607011000 / CLAUDE.md Actors section). One graph run = one
  administration operation request (intake → advise → govern → decide →
  commit/hold, with a human-approval interrupt for escalated proposals).
  No infinite internal loop; checkpointed per superstep so an
  ...
  The unconditional invariant: the AdminManagementAdvisor can never
  directly commit a record or dispatch a robot action the
  BusinessAdminGovernor refuses — every commit-record! call is gated
  behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [business-admin.advisor :as advisor]
            [business-admin.governor :as governor]
            [business-admin.store :as store]))

(defn build-graph
  "Build a compiled AdminManagementActor graph. `store` implements
  `business-admin.store/Store`. `advisor` implements
  `business-admin.advisor/Advisor` (defaults to `mock-advisor`).
